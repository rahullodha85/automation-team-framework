...
import ui.pages.repo.sectionsmobile.SaksCartRepo;
...
return false;
        ...
        if(getSaksItemsInCart().size()==index)
            return;
                    while(true) {
                        ...
                        driver.get(Config.getApplicationUrl()+"checkout/checkout.jsp#review");
    ...
    public void updateCart(String s, String s1) throws Exception {
        String regexItemId="Item-(\\d+)\".*?"+s.toString();
        Object cartItemId= regexSearch.getString(regexItemId,getItem(s).getAttribute("outerHTML"));
        makeRequest("checkout/checkout.jsp?bmForm=update_cart_item_quantity_service&cartItemId="+cartItemId.toString()+"&itemQuantity="+s1.toString());
